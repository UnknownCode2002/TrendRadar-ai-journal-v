...
def generate_html_report(
    ...
    if report_metadata:
        _METADATA_KEYS = {
            ...
            "collect_count", "github_trending",
        }
        for key in _METADATA_KEYS:
            if key in report_metadata:
                report_data[key] = report_metadata[key]
        # debug
        gh_in_rd = "github_trending" in report_data
        gh_len = len(report_data.get("github_trending", []))
        print(f"[GH GEN] key in report_data={gh_in_rd}, len={gh_len}")
